...
public class BankTransaction {
    ...
    public BankTransaction(long trxValue) {
        if (trxValue <= MIN_TRX_VALUE || trxValue >= MAX_TRX_VALUE) {
            throw new IllegalArgumentException("This transaction is out of transaction limit! " + trxValue);
        }
        countTrx++;
    ...
    public static BigDecimal averageOfTransaction() {
        return countTrx == 0 ? new BigDecimal("0") : sumOfTrxs.divide(new BigDecimal(Long.toString(countTrx)),
